...
begin
  ...
  if folders[1] = 'messages' then
    legacy_message_id := regexp_replace(storage.filename(object_name), '-[0-9]+[.]jpg$', '');
...
create policy "conversation members read message images" on storage.objects
for select to authenticated using (
  bucket_id = 'message-images'
  and public.can_access_message_image(name)
);
create policy "conversation members upload message images" on storage.objects
